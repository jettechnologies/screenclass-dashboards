...
// Safe sessionStorage access with SSR check
...
const saveStateToSessionStorage = (state: QuizState) => {
  safeSessionStorage.setItem("quiz-storage", JSON.stringify(state));
};

const loadStateFromSessionStorage = (): QuizState | null => {
  const item = safeSessionStorage.getItem("quiz-storage");
  return item ? JSON.parse(item) : null;
};
...
export const createQuizSlice: StateCreator<
  ...
  QuizSlice
> = (set, get) => {
  // Load initial state from sessionStorage (safe for SSR)
  const initialState = loadStateFromSessionStorage() || {
    quizData: null,
    userAnswers: [],
  };
  ...
  return {
    ...
    actions: {
      ...
      resetQuiz: () => {
        set((state) => {
          const newState = { ...state, quizData: null, userAnswers: [] };
          safeSessionStorage.removeItem("quiz-storage");
          return newState;
        });
      },
      getQuizById: (quizId) => {
