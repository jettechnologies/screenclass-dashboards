...
export const GradeLevel = () => {
  return (
    <section className="grid min-h-fit w-full place-items-center bg-[#F4F4F4] px-2 pb-8 pt-16 font-poppins sm:px-8 md:min-h-screen md:pb-0 md:pt-0 lg:px-14">
      <div className="min-h-3/5 relative flex w-full flex-col gap-x-4 lg:flex-row">
        ...
        <div className="z-30 mt-16 flex h-full flex-col gap-x-0 gap-y-6 md:gap-y-0 md:mt-0 md:flex-row md:gap-x-6 lg:z-0 lg:gap-x-8 items-center">
          {data.map((item, index) => (
            <div
              className="flex-1 rounded-sm bg-white w-[80%] lg:h-[500px] lg:w-[282px]"
              key={index}
            >
              <GradeCard {...item} />
            </div>
